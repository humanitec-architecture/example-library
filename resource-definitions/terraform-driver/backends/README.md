...
Backend configuration can vbe defined in the [`terraform` block](https://developer.hashicorp.com/terraform/language/v1.5.x/settings). Terraform imposes limitations on the `terraform` block. This means that variables cannot be used to parameterize the `terraform` block. 
...
The Platform Orchestrator supports Temporary Credentials via Cloud Accounts. All backends can be configured via environment variables, so it is recommended to use the [`credentials_config` object](https://developer.humanitec.com/integration-and-extensions/drivers/generic-drivers/terraform/#inputs) to specify credentials via the appropriate environment variable for the backend.
...
- [S3 backend using temporary credentials](./s3)
